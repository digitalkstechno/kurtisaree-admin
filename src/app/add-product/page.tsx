...
import { Plus, LayoutDashboard, Package, LogOut, Loader2, Upload, X, ChevronLeft, Sparkles, Save, Info } from 'lucide-react';
...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to publish creation');
    } finally {
      setLoading(false);
    }
  };
      ...
      <aside className="w-72 bg-gray-900 text-white hidden lg:flex flex-col sticky top-0 h-screen shadow-2xl z-20">
        ...
        <div className="px-4 mb-4">
          ...
          <nav className="space-y-1">
            ...
            <Link href="/add-product" className="flex items-center gap-3 px-4 py-3.5 bg-white/10 text-white rounded-xl font-bold transition-all border border-white/5">
              <Plus size={20} className="text-pink-500" /> Add Saree/Kurti
            </Link>
          </nav>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 px-8 py-6 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-serif text-gray-900 tracking-tight">Add Saree / Kurti Details</h1>
              <p className="text-sm text-gray-400 font-medium uppercase tracking-widest mt-1">Enter details for the catalog</p>
            </div>
          </div>
          <button
